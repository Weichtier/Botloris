...
public class ClientDbIdCommand implements Command {
    @Override
    public boolean execute(Client client, String s, String[] args) {
        if(args.length >= 1){
            if(Utils.isAdmin(client)){
                ...
                Main.getInstance().getApi().sendPrivateMessage(client.getId(), "Database ID of User " + BBCode.BOLD_START + username + BBCode.BOLD_END + ": " + BBCode.BOLD_START + target.getDatabaseId() + BBCode.BOLD_END +  "");
            }else {
                Main.getInstance().getApi().sendPrivateMessage(client.getId(), Utils.noPermissions);
            }
        }else {
            Main.getInstance().getApi().sendPrivateMessage(client.getId(), "Your Database ID: " + BBCode.BOLD_START + client.getDatabaseId() + BBCode.BOLD_END);
        }
        return false;
    }
}
